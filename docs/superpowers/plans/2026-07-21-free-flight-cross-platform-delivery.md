...
Apollo Neuro 调研表明，消费级“柔和触觉振动 + 场景化调节”可作为随心飞的补充交互，但当前不应把它设为首版随心飞的硬件依赖。详见 docs/research/2026-07-21-apollo-pressure-wearable-research.md。

- 首版：音频、动画、呼吸引导和会话摘要为完整闭环；无论手环是否支持精细触觉，都可运行。
- WearableKit：预留可选 HapticFeedbackPort，由设备能力声明支持的模式、强度、延迟和安全停止；不支持时自动降级。
- 二期实验：只验证触觉引导呼吸，比较“音频+视觉”与“音频+视觉+触觉”的舒适度、完成率和短期自评；不宣称治疗或迷走神经刺激。
- 三期个体化：仅在原生 PPI/RRI 和个人基线经过验证后，再研究触觉模式与恢复趋势的匹配。
